...
class CustomKernel(Kernel):
    ...
    def forward_left(self, x1, Dx1, x2, Dx2, last_dim_is_batch=False):
        x1_ = x1 * self.variance.sqrt()
        x2_ = x2 * self.variance.sqrt()
        Dx1_ = Dx1 * self.variance.sqrt()
        Dx2_ = Dx2 * self.variance.sqrt()

        # prod = MatmulLazyTensor(x1_, x2_.transpose(-2, -1))

        Dx1_ = Dx1_.transpose(0, 1)  ### ??? should I transpose?

        prod = MatmulLazyTensor(Dx1_, x2_.transpose(-2, -1))


        return prod

    def forward_right(self, x1, Dx1, x2, Dx2, last_dim_is_batch=False):
        x1_ = x1 * self.variance.sqrt()
        x2_ = x2 * self.variance.sqrt()
        Dx1_ = Dx1 * self.variance.sqrt()
        Dx2_ = Dx2 * self.variance.sqrt()
        ...
        Dx2_ = Dx2_.transpose(0, 1)  ### ??? should I transpose?

        prod = MatmulLazyTensor(x1_, Dx2_.transpose(-2, -1))

        return prod

    def forward_left_right(self, x1, Dx1, x2, Dx2, last_dim_is_batch=False):
        x1_ = x1 * self.variance.sqrt()
        x2_ = x2 * self.variance.sqrt()
        Dx1_ = Dx1 * self.variance.sqrt()
        Dx2_ = Dx2 * self.variance.sqrt()
        ...
        Dx_ = Dx1_.transpose(0, 1)  ### ??? should I transpose?

        prod = MatmulLazyTensor(Dx1_, Dx2_.transpose(-2, -1))
