...
class Predictor:
    ...
    def predict_and_plot_energies(self):
        ...
        test_x_e, test_y_e = get_vectors_e(
            self.test_X, self.test_Y, self.n_molecules, self.n_atoms
        )
    ...
    def predict_and_plot_forces(self):
        ...
        predictions = res.mean.cpu().detach().numpy()
        actual_values = self.test_Y.cpu().detach().numpy()
        plt.rcParams["figure.figsize"] = (30, 10)
        ...
        # mol = self.mol_traj[0]
        predicted_forces = (
            predictions[: -self.n_molecules ]
            .reshape(self.n_atoms, -1)
            .transpose(1, 0)
        )

        upper_forces = (
            upper[: -self.n_molecules]
            .reshape(self.n_atoms, -1)
            .transpose(1, 0)
        )
        ...
        actual_energies = self.test_Y[-self.n_molecules :]
        ...
        FMIN = self.test_Y.min()
        FMAX = self.test_Y.max()
